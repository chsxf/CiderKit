...
public class TransformAssetElementView : NSStackView, NSTextFieldDelegate, FloatFieldDelegate, IntFieldDelegate {
    ...
    private let flippedCheckbox: NSButton
    ...
    required init(assetDescription: AssetDescription, element: TransformAssetElement) {
        ...
        flippedCheckbox = NSButton(checkboxWithTitle: "Horizontally Flipped", target: nil, action: #selector(Self.flippedCheckboxClicked))
        ...
        assetElementViews = [
            ...
            VSpacer(), InspectorHeader(title: "Offset"), offsetRow, flippedCheckbox
        ]
    ...
    @objc
    private func flippedCheckboxClicked() {
        if let elementViewDelegate, let element {
            element.flipped = flippedCheckbox.state == .on
            elementViewDelegate.update(element: element)
        }
    }
